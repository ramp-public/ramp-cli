...
- The tools are gated by `RAMP_CLI_HOTEL_BOOKING`. If Core reports that the feature is disabled,
  explain that hotel search and booking are not enabled for this Ramp account. Do not fall back
...
as `--cursor` and a rationale; omit the original search fields because Core ignores them and reads
...
`policy_summary` before the table when present; it is Core's authoritative summary of what this
traveler may book.
...
For delegated booking, pass the same `--traveler_user_id`. If Core says the rate expired or is
...
- matching `loyalty_programs`: meaningful `display_name` and `loyalty_number` values that Core will
  submit on confirm
...
matching membership Core will submit, rather than treating the rate's generic program name as
proof of enrollment.

If the preview is out of policy, show every returned violation and ask why the traveler needs that
rate. Keep their exact justification as `oop_reason`; Core requires `--oop_reason` on confirmation
...
`--trip_id` during preview: Core only resolves it during confirm. A missing, invalid, or other-user
trip is ignored and Core auto-selects or creates one. Verify the resulting `trip_id`/`trip_name`
...
- `--trip_id '<trip_uuid>'`: exact existing trip UUID explicitly selected by the traveler; omit to
  let Core auto-select or create a trip. This is the step where Core resolves it.
