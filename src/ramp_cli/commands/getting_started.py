...
from ramp_cli.tools.registry import CATEGORY_REMAP, list_categories
...
def _remap_categories(categories: dict[str, list]) -> dict[str, list]:
    ...
    E.g. ``cards`` and ``agent_cards`` are merged into ``funds`` so that
    the onboarding guide shows names that match invokable ``ramp <resource>``
    groups.
    """
